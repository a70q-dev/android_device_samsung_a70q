...
#define LOG_TAG "android.hardware.biometrics.fingerprint@2.3-service-a70q"
...
BiometricsFingerprint::BiometricsFingerprint() : mClientCallback(nullptr) {
    ...
    if (getBootloader().find("A715") != std::string::npos) {
        set(TSP_CMD_PATH, "set_fod_rect,421,2018,659,2256");
    } else if (getBootloader().find("A715MN") != std::string::npos) {
        set(TSP_CMD_PATH, "set_fod_rect,426,2031,654,2259");
    } else {
        LOG(ERROR) << "Device is not an A70 or A70MN, not setting set_fod_rect";
    }
